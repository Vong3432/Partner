...
import { showProfile, editProfile, addEducationInfo, getEducationInfo, deleteEducation, addExperienceInfo, getExperienceInfo, deleteExperience, getResumes, deleteResume } from '../../actions/profileActions'
...
const EditProfile = (props) => {
    ...
    const onSubmit = (e) => {
        ...
        if (pdfFile) {
            ...
            for (var x = 0; x < pdfFile.length; x++) {
                console.log(pdfFile[x].name)
                data.append('file', pdfFile[x])
            }
            axios
                .post('/api/profile/uploadPDF', data)
        ...
        <>
            ...
            <section className="profile-section mx-auto" style={{ maxWidth: "900px" }}>
                ...
                <div className="p-5 card-shadow profile--about-container" style={part}>
                    ...
                    {!isEmployer && (<div className="d-flex flex-row mt-3">
                ...
                {(!isEmployer && isLoading === false) && (
                    <div className="p-5 my-3 card-shadow d-flex flex-column" style={part}>
                        ...
                        {resumes.map((item, index) => (
                            <>
                                <div className="d-flex flex-row">
                                    <a style={{wordBreak:"break-all"}} className="mb-2" download href={`../../../../../uploads/profile/resume/${props.match.params.id}-${item.Name}`}>{item.Name}</a>
                                    <img className="small-icon ml-auto" onClick={e => onDeleteResume(e, item.ResumeID)} style={{ width: "20px", height: "20px", cursor: "pointer" }} src={require('../../images/delete.svg')} alt="delete" />
                                </div>                                
                            </>
                        ))}
                        <div className="input-group">
                            ...
                            <div className="custom-file">
                                <input type="file" onChange={e => onEdit(e)} multiple name="pdfUpload" className="custom-file-input" id="inputGroupFile01" aria-describedby="inputGroupFileAddon01" />
